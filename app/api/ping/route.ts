...
import { upsertSession, checkAndClearKick } from "@/lib/stats";
...
export async function POST(req: NextRequest) {
  ...
  const keyUsed = typeof body.key === "string" ? body.key.slice(0, 40) : "";
  ...
  try {
    await upsertSession(jobId, placeId, userId, playerName, displayName, playerCount, executor, executorVersion, keyUsed);
    const kick = await checkAndClearKick(jobId, userId);
    return NextResponse.json({ ok: true, kick });
  } catch (e) {
    console.error("juru.lol /api/ping failed:", e);
