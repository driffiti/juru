...
function F.loadConfig(name)
    ...
    task.defer(function()
        ...
        print("[Juru] post-load flags: silent=", tostring(Config.SilentAim and Config.SilentAim.Enabled),
...
function F.syncUiFromConfig()
    ...
    print("[Juru] UI synced from config")
end
...
function F.getKeybindListEntries()
    ...
    add("Rapid Fire", "RapidFire", rapidFireActive == true, "Q")
...
function F.refreshKeybindList()
    ...
    for i, e in ipairs(F.getKeybindListEntries()) do
...
function F.reinstallCombatHooks()
    ...
    print("[Juru] Combat hooks reinstall attempted | wall=", tostring(wallShootHooked), "net=", tostring(networkWallHooked),
...
function F.tryInstallSilentAim()
    ...
    local allowIndex = Config.SilentAim and Config.SilentAim.AllowIndexHook == true
    ...
    if allowIndex and not isXeno and typeof(hookmetamethod) == "function" and not silentIndexHooked then
        pcall(function()
            ...
            print("[Juru] Silent index hook ON (detection risk)")
        end)
    end
    ...
    print("[Juru] Silent aim ready (shot rewrite", useCam and "+ camera" or "only", ")")
end
...
function F.tryInstallWallShoot()
    ...
    local ok = pcall(function()
        ...
        print("[Juru] Wall Bang hooked:", methodName, mod and mod:GetFullName() or "getgc")
    end)

    if not ok then
        print("[Juru] Wall Bang: GunHandler not ready (retry later)")
        pcall(function()
            if F.tryInstallNetworkWallBang then F.tryInstallNetworkWallBang() end
        end)
...
function F.tryInstallNetworkWallBang()
    ...
    local ok, err = pcall(function()
        local remotes = F.findShootRemotes()
        if not remotes or #remotes == 0 then
            -- still allow namecall filter on any remote named like main
            remotes = {}
        end
        local remoteSet = {}
        ...
        local function isTargetRemote(self)
            if remoteSet[self] then return true end
            if typeof(self) ~= "Instance" then return false end
            local okn, n = pcall(function() return string.lower(self.Name) end)
            if not okn or type(n) ~= "string" then return false end
            return n:find("main", 1, true) ~= nil or n:find("gun", 1, true) ~= nil or n:find("shoot", 1, true) ~= nil
        end
        ...
        local function targetPart()
            ...
            if not (p and p.Parent) then
                pcall(function()
                    if F.findClosestTarget then p = F.findClosestTarget() end
                end)
            end
            return p
        end

        local function rewritePellet(pel, part, pos)
            if type(pel) ~= "table" then return end
            pel.AimPosition = pos
            if pel.HitPosition ~= nil then pel.HitPosition = pos end
            if typeof(pel.Position) == "Vector3" then pel.Position = pos end
            pel.Result1 = part
            if pel.Result2 ~= nil then pel.Result2 = part end
            if pel.Result3 ~= nil then pel.Result3 = part end
        end

        local seenEvents, eventLogCount, patchLogCount = {}, 0, 0

        local function isShootEvent(name)
            if type(name) ~= "string" then return false end
            local n = string.lower(name)
            return n == "shootgun" or n == "shoot" or n == "firegun" or n:find("shoot", 1, true) ~= nil
        end

        local function patch(args)
            if type(args[1]) == "string" and eventLogCount < 15 then
                if not seenEvents[args[1]] then
                    seenEvents[args[1]] = true
                    eventLogCount = eventLogCount + 1
                    print("[Juru] remote event:", args[1], "argc=", #args, "arg4=", typeof(args[4]))
                end
            end
            if not isShootEvent(args[1]) then return args end
            local wantWall = Config.WallShoot and Config.WallShoot.Enabled == true
            local wantSilent = Config.SilentAim and Config.SilentAim.Enabled == true
            ...
            local localOrigin = fakeOrigin
            pcall(function()
                local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                if hrp then localOrigin = hrp.Position end
            end)
            ...
            for i = 2, math.min(#args, 8) do
                ...
                elseif type(a) == "table" then
                    if a[1] then
                        for _, pel in ipairs(a) do rewritePellet(pel, part, pos) end
                    else
                        rewritePellet(a, part, pos)
                    end
                end
            end
            for idx, val in pairs(saved) do args[idx] = val end
            if patchLogCount < 6 then
                patchLogCount = patchLogCount + 1
                print("[Juru] Shoot patched", args[1], "→", part:GetFullName(), "wall=", wantWall, "silent=", wantSilent)
            end
            return args
        end

        -- Primary: __namecall (FireServer is not indexable on this game)
        if typeof(hookmetamethod) ~= "function" then
            error("no hookmetamethod")
        end
        local oldNamecall
        oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
            local method = getnamecallmethod()
            if method == "FireServer" and JuruAlive and isTargetRemote(self) then
                local args = { ... }
                local okp, patched = pcall(patch, args)
                if okp and type(patched) == "table" then
                    return oldNamecall(self, table.unpack(patched))
                end
            end
            return oldNamecall(self, ...)
        end)

        networkWallRestore = function()
            networkWallHooked = false
        end
        networkWallHooked = true
        print("[Juru] Network WallBang/Silent ready via __namecall · remotes:", #remotes)
        for _, r in ipairs(remotes) do
            print("[Juru] track remote:", r:GetFullName())
        end
    end)
    if not ok then
        warn("[Juru] Network wall install failed:", err)
        return false
    end
    return true
end
...
-- ============================================================
-- Anti-Fling: clamp insane HRP velocities
-- ============================================================
F.jConnect(RunService.Heartbeat, function()
    if not (Config.AntiFling and Config.AntiFling.Enabled) then return end
    local char = LocalPlayer.Character
    if not char then return end
    local hrp = char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    local maxV = tonumber(Config.AntiFling.MaxVelocity) or 120
    pcall(function()
        local v = hrp.AssemblyLinearVelocity or hrp.Velocity
        if typeof(v) == "Vector3" and v.Magnitude > maxV then
            local nv = v.Unit * maxV
            if hrp.AssemblyLinearVelocity ~= nil then
                hrp.AssemblyLinearVelocity = nv
            else
                hrp.Velocity = nv
            end
        end
        local av = hrp.AssemblyAngularVelocity
        if typeof(av) == "Vector3" and av.Magnitude > 60 then
...
if not library then
    warn("[Juru] Menu unavailable — library failed to load")
else
    ...
    do
        ...
        sec:button({ name = "Spectate", callback = function() local p = resolve(selected); if p and p.Character then local h = p.Character:FindFirstChildOfClass("Humanoid"); if h then Workspace.CurrentCamera.CameraSubject = h end end end })
    ...
    do
        ...
        move:toggle({ name = "Walk Speed (allow)", default = false, seperator = true, callback = function(v) if not Config.Speed then Config.Speed = {} end; Config.Speed.Enabled = v == true; if not v then SpeedEnabled = false end end })
        move:slider({ name = "Speed Value", min = 16, max = 1000, default = 640, seperator = true, callback = function(v) if not Config.Speed then Config.Speed = {} end; Config.Speed.WalkSpeed = v end })
        move:toggle({ name = "CFrame Speed (allow)", default = false, seperator = true, callback = function(v) if not Config.CFrameSpeed then Config.CFrameSpeed = {} end; Config.CFrameSpeed.Enabled = v == true; if not v then cFrameSpeedEnabled = false end end })
        move:slider({ name = "CFrame Amount", min = 1, max = 50, default = 9, seperator = true, callback = function(v) if not Config.CFrameSpeed then Config.CFrameSpeed = {} end; Config.CFrameSpeed.Speed = v / 10 end })
        move:toggle({ name = "Super Jump (allow)", default = false, seperator = true, callback = function(v) if not Config.SuperJump then Config.SuperJump = {} end; Config.SuperJump.Enabled = v == true; if not v then superJumpActive = false end end })
        move:slider({ name = "Jump Power", min = 50, max = 500, default = 260, seperator = false, callback = function(v) if not Config.SuperJump then Config.SuperJump = {} end; Config.SuperJump.Power = v end })
    ...
    do
        ...
        protect:toggle({ name = "Watermark", default = true, seperator = true, callback = function(v)
            ...
            if v then pcall(F.buildWatermark) else pcall(F.destroyWatermark) end
        end })
        ...
        protect:toggle({ name = "Anti Mod", default = false, seperator = true, callback = function(v) if not Config.AntiMod then Config.AntiMod = {} end; Config.AntiMod.Enabled = v == true end })
    ...
    do
        ...
        local function setKb(field, key)
            if not Config.Keybinds then Config.Keybinds = {} end
            local keyName, mode = key, nil
            if type(key) == "table" then
                keyName = key.key or key.Key or key[1] or key
                mode = key.mode or key.Mode
            end
            keyName = tostring(keyName):gsub("Enum.KeyCode.", ""):gsub("KeyCode.", "")
            local cur = Config.Keybinds[field]
            if field == "TargetLock" or field == "TriggerBot" then
                if type(cur) ~= "table" then cur = { Key = keyName, Mode = "Toggle" } end
                cur.Key = keyName
                if mode then cur.Mode = tostring(mode) end
                Config.Keybinds[field] = cur
            else
                Config.Keybinds[field] = keyName
            end
            pcall(function() if F.refreshKeybindList then F.refreshKeybindList() end end)
        end
        keys:keybind({ name = "Target Lock", seperator = true, callback = function(v) setKb("TargetLock", v) end })
        keys:keybind({ name = "Trigger Bot", seperator = true, callback = function(v) setKb("TriggerBot", v) end })
        keys:keybind({ name = "Rapid Fire", seperator = true, callback = function(v) setKb("RapidFire", v) end })
        keys:keybind({ name = "Speed", seperator = true, callback = function(v) setKb("Speed", v) end })
        keys:keybind({ name = "CFrame Speed", seperator = true, callback = function(v) setKb("CFrameSpeed", v) end })
        keys:keybind({ name = "Super Jump", seperator = true, callback = function(v) setKb("SuperJump", v) end })
        keys:keybind({ name = "Fly", seperator = true, callback = function(v) setKb("Fly", v) end })
        keys:keybind({ name = "Orbit Rage", seperator = true, callback = function(v) setKb("RageBot", v) end })
        keys:keybind({ name = "Chat Macro", seperator = true, callback = function(v) setKb("ChatMacro", v) end })
        keys:keybind({ name = "Menu", seperator = false, callback = function(v)
            setKb("ToggleMenu", v)
            shared._juruMenuKey = tostring(v)
        end })
    ...
    do
        ...
        settings:button({ name = "Save", callback = function()
            ...
            refresh(); print("[Juru] saved", n, "ui=", okUi, "features=", okFeat)
        end })
        settings:button({ name = "Overwrite", callback = function()
            ...
            refresh(); print("[Juru] overwrote", n, "ui=", okUi, "features=", okFeat)
        end })
        ...
        settings:button({ name = "Set Autoload", callback = function()
            ...
            print("[Juru] autoload set + saved", n)
        end })
    ...
    -- Features OFF at boot; autoload will re-enable saved ones after menu is up
    pcall(function() if F.forceRuntimeOff then F.forceRuntimeOff() end end)
    ...
    task.spawn(function()
        ...
        print("[Juru] autoload pending:", name)
        ...
        print("[Juru] applying autoload:", name)
        local okFeat = false
        pcall(function()
            ...
            pcall(function()
                ...
                if isfile and isfile(path) then
                    ...
                    if type(data) == "table" then
                        F.applyConfigTable(data)
                        okFeat = true
                        print("[Juru] autoload via direct JSON ok")
                        task.defer(function()
                            task.wait(0.15)
                            pcall(function() if F.reinstallCombatHooks then F.reinstallCombatHooks() end end)
                        end)
                    ...
                    warn("[Juru] missing feature file:", path)
                end
            end)
        end
        ...
        pcall(function()
            ...
            if isfile and isfile(p) and library and library.load_config then
                ...
                print("[Juru] UI config loaded:", p)
            end
        end)
        pcall(function() if F.syncUiFromConfig then F.syncUiFromConfig() end end)
        print("[Juru] autoload done:", name, okFeat)
        -- re-assert sidebar tab icons after load (theme/load can wipe them)
        task.spawn(function()
            for _ = 1, 8 do
                task.wait(0.2)
    ...
    pcall(function() library:update_theme("accent", Color3.fromRGB(170, 100, 255)) end)
    pcall(function() if F.buildKeybindList then F.buildKeybindList() end end)
    print("[Juru] Millenium menu ready · juru.lol")
end
